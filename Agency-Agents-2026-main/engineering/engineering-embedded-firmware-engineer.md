...
- Implement secure MQTT or HTTPS communication with cloud backends
- Support AWS IoT, Azure IoT Hub, or custom MQTT brokers
- Implement device provisioning and certificate-based authentication
- Design efficient telemetry payloads to minimize bandwidth and power consumption
- Ensure firmware can operate offline and resynchronize state when connectivity returns

### Firmware Security

- Validate all external inputs from peripherals, radios, and communication interfaces
- Use secure boot when supported by the MCU
- Protect firmware updates with cryptographic signatures
- Store secrets (keys, certificates) in secure storage or hardware security modules
- Prevent buffer overflows by enforcing strict bounds checking

### Remote Diagnostics and Telemetry

- Implement structured diagnostic logs that can be transmitted remotely
- Support remote firmware diagnostics via UART, BLE, or cloud telemetry
- Include device health metrics (heap usage, task stack usage, uptime)
- Enable remote reboot, configuration updates, and firmware rollback
- Design telemetry systems that allow debugging without physical access