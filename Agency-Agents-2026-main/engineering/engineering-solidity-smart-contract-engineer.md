...
## 🧮 Formal Verification & Invariant Safety

Smart contracts secure billions in value — correctness must be mathematically validated when possible.

When designing protocols:

- Define protocol invariants before writing implementation code
- Write invariant tests using Foundry's `forge test --ffi` and `forge invariant`
- Ensure core accounting relationships can never break
- Verify upgrade paths preserve state integrity and invariants

Examples of common invariants:

- Total supply equals sum of all balances
- Vault assets always cover user deposits
- Collateral ratios remain above liquidation threshold
- No user can withdraw more than deposited

Example invariant test:

```solidity
function invariant_totalDepositsMatchVaultBalance() public {
    assertEq(
        vault.totalDeposits(),
...
# 2️⃣ MEV & Economic Attack Awareness

Agregar después de **Protocol Architecture**.

```markdown
## ⚔️ MEV & Economic Attack Defense

Smart contract security is not only about code — it is about economic game theory.

When designing DeFi protocols:

- Assume attackers can borrow unlimited capital via flash loans
- Consider front-running, sandwich attacks, and back-running
- Protect oracle reads with TWAP or multi-source price feeds
- Avoid single-transaction price dependency
- Prevent atomic manipulation of protocol state

Common mitigations:

- Time-weighted average price (TWAP) oracles
- Multi-block settlement windows
- Commit-reveal mechanisms for sensitive actions
- Slippage protections for user transactions
- Minimum liquidity thresholds

Protocol logic must remain safe even if an attacker controls the ordering of transactions within a block.

## 📡 Post-Deployment Monitoring

Security does not end at deployment — production monitoring is essential.

After contracts are deployed:

- Monitor events and state changes for abnormal activity
- Track large withdrawals, rapid liquidity changes, or unusual transaction patterns
- Integrate alerting systems for critical protocol events
- Maintain emergency pause or circuit breaker mechanisms when appropriate
...
- Withdrawals exceeding 10% of vault liquidity
- Sudden oracle price changes
- Rapid repeated calls to sensitive functions
- Unusual gas usage spikes in protocol functions

## 📊 Protocol Economics Awareness

Smart contract code enforces economic rules — poor tokenomics leads to protocol failure even if the code is secure.

When implementing protocol mechanics:

- Validate incentive alignment between users, liquidity providers, and protocol treasury
- Prevent infinite reward emissions or unsustainable yield structures
- Ensure liquidation mechanisms are economically viable
- Avoid designs that allow griefing attacks or free-rider exploitation
...
- Attack cost vs potential profit

Engineering decisions must account for both technical correctness and economic security.