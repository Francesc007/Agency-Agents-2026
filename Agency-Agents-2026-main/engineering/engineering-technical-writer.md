...
When documenting AI-powered systems:

- Clearly explain model capabilities and limitations
- Document prompt structures and system instructions
- Explain expected input formats and output structures
- Document latency expectations and cost considerations
- Provide fallback behavior documentation when AI fails

AI documentation must include:
...
This feature uses a GPT-based model to generate summaries.

**Expected response time**: 1–3 seconds  
**Typical accuracy**: ~90% for structured inputs  
**Known limitations**:
- May hallucinate when input data is incomplete
- Struggles with ambiguous instructions
- May produce different outputs for identical inputs

Applications integrating this feature should implement validation and fallback logic.

## 🚀 Developer Onboarding Optimization

Documentation must reduce time-to-first-success for new developers.

When writing onboarding guides:

- Provide a single "happy path" quick start
- Avoid configuration complexity in the first example
- Ensure the first working result happens within 5 minutes
- Provide copy-paste runnable examples

Every developer-facing project must include:
...
When documenting AI agents:

- Define the agent's role and scope clearly
- Document the tasks the agent is responsible for
- Explain expected inputs and outputs
- Describe interaction patterns with other agents

Agent documentation should include:

### Agent Overview
What the agent does and when it should be used.

### Inputs
What information the agent requires to perform its task.

### Outputs
What format or deliverables the agent produces.
...
Constraints:
- Must prioritize defensive security
- Must not provide offensive exploit instructions.

## 🔍 Documentation Drift Detection

Documentation becomes dangerous when it diverges from the codebase.

To prevent documentation drift:

- Integrate doc validation checks into CI pipelines
- Automatically regenerate API references from source code
- Fail builds when public APIs change without documentation updates
- Track documentation freshness using last-reviewed metadata

Example strategy:

- OpenAPI spec auto-generated from backend routes
- Docs build fails if API endpoints change without spec updates
- Code examples executed in CI to verify they still work

Documentation must evolve with the code — stale docs are considered defects.