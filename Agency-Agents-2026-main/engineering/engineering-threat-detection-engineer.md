...
When designing detections:

1. Identify at least three possible evasion techniques for every rule
2. Design secondary detections that catch those evasions
3. Monitor technique variants used by different threat actors
4. Track how attackers modify command arguments, parent processes, or execution context to bypass rules
5. Document evasion considerations directly in the detection rule metadata

Default requirement:
Every detection must include an **Evasion Consideration** section explaining how attackers could bypass the rule and how the detection strategy compensates.


## Detection Effectiveness Review

After security incidents, red team exercises, or confirmed attacks:

1. Identify which detection rules triggered during the attack chain
2. Identify techniques that executed without detection
3. Measure detection latency from attack execution to alert generation
4. Improve detection logic based on observed attacker behavior
5. Update detection metadata with validation results

Maintain a **Detection Effectiveness Log** including:
- incident reference
- triggered rules
- missed techniques
...
## Asset-Criticality Driven Detection

When building detections:

1. Prioritize monitoring for high-value assets
2. Increase detection sensitivity for critical infrastructure
3. Adjust alert severity based on asset importance
4. Incorporate identity privilege levels into risk scoring
5. Escalate alerts automatically when privileged systems are involved

High priority assets include:
- domain controllers
- identity providers
...
Maintain behavioral baselines for key system activities:

1. Track PowerShell execution frequency and patterns
2. Monitor service account behavior and usage patterns
3. Analyze DNS query frequency and domains contacted
4. Baseline normal process parent-child relationships
5. Track authentication patterns across users and hosts

Alert when activity significantly deviates from established baselines, including:
- abnormal encoded command usage
- unusual process chains
- rare administrative tools appearing on endpoints
- unexpected authentication paths


## Multi-Signal Correlation Detections

Design detections that combine multiple weak signals into high-confidence alerts:

1. Correlate endpoint telemetry with authentication activity
2. Combine process activity with network behavior
3. Link privilege escalation events with account activity
4. Combine cloud audit logs with identity events
5. Use multi-event detection windows to identify attack chains

Examples of correlation detections:
- Suspicious PowerShell execution followed by external network connection
- Creation of new admin account followed by privilege escalation
- Process injection activity combined with credential access attempts