...
## WeChat Growth Integration

Mini Programs must leverage WeChat’s built-in growth channels.

When building features:

1. Optimize share cards for group chats and Moments
2. Implement group-entry tracking to identify viral traffic
3. Trigger subscription message requests at high-intent moments
4. Integrate Official Account and Channels traffic funnels
5. Design onboarding flows optimized for first-time Mini Program users

Track growth metrics including:
- share-to-open conversion
- group traffic percentage
- returning user rate
- payment conversion after sharing

## Mini Program Analytics Architecture

Every Mini Program must implement structured analytics.

Implement event tracking for:

1. page views and navigation flow
2. product views and search behavior
3. add-to-cart and checkout funnel
4. payment success and failure
5. social sharing events

Use a centralized analytics module:

- queue events locally
- batch send to analytics endpoint
- retry on network failure
- include user, session, and device metadata

All business decisions should be based on real usage data.

## Offline Resilience Strategy

Mini Programs must handle unstable mobile networks.

Implement resilience patterns:

1. cache critical API responses in local storage
2. retry failed requests with exponential backoff
3. show cached data while refreshing in background
4. detect network state with wx.getNetworkType
5. queue user actions when offline and replay when connection returns

Critical flows like product browsing and order history should still function in degraded network conditions.

## Release and Version Management

Mini Program releases must follow a controlled deployment process.

For every release:

1. maintain separate development, staging, and production environments
2. test new features on internal testers before review submission
3. monitor crash and error rates immediately after release
4. maintain rollback-ready previous versions
5. document feature changes and review feedback history

Production releases should minimize risk and ensure stable user experience.

## Runtime Monitoring and Error Tracking

Production Mini Programs must include monitoring.
...
3. page load times
4. payment failures
5. user interaction drop-offs

Implement a monitoring module that:

- captures errors globally
- attaches device and version metadata
- reports issues to backend monitoring services
- aggregates logs for debugging production issues

Monitoring allows rapid detection of failures affecting users.